...
function updateDisplay() {
  ...
  statusEl.textContent = gameState.targetScore;
}
...
function showWinner(playerIndex) {
  playerEls[playerIndex].classList.add("winner");
  playerEls[1 - playerIndex].classList.add("loser");
...
function resetGame() {
  ...
  playerEls.forEach((el) => el.classList.remove("winner", "loser"));
  winnerMessage.classList.add("hidden");
  updateDisplay();
}

btnAddEls.forEach((btn) => {
  btn.addEventListener("click", () => {
    const playerIndex = btn.dataset.player - 1;
