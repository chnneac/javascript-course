...
function buildForecast(
  arr,
  { startDay = 1, sep = ' ... ', lead = '... ', tail = ' ...' } = {}
) {
  if (!Array.isArray(arr) || arr.length === 0) return '...';
  const parts = arr.map((t, i) => {
    const day = startDay + i;
    return `${t}°C in ${day} ${day === 1 ? 'day' : 'days'}`;
  });
  return `${lead}${parts.join(sep)}${tail}`;
}

function printForecast(arr, opts) {
  console.log(buildForecast(arr, opts));
}

// Tests
printForecast([17, 21, 23]);
printForecast([12, 5, -5, 0, 4]);
printForecast([30], { startDay: 5 });
